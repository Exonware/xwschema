        ...
        try:
            with self.performance_monitor.monitor_operation(f"schema_{operation}"):
                ...
                if operation == "validate":
                    ...
                    return self.parser.parse_schema(schema, **kwargs)
...
class SchemaParser:
    ...
    def parse_schema(
        ...
        try:
            ...
            if isinstance(schema_source, Path) or (isinstance(schema_source, str) and Path(schema_source).exists()):
                # Parse from file
                return self._parse_from_file(Path(schema_source), format)
            
            elif isinstance(schema_source, str):
                # Parse from string
                return self._parse_from_string(schema_source, format)
    ...
    def _parse_from_file(self, file_path: Path, format: Optional[str]) -> Dict[str, Any]:
        """Parse schema from file."""
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
            return self._parse_from_string(content, format or file_path.suffix[1:])
        except Exception as e:
            raise XSchemaParsingError(f"Failed to parse schema from file: {file_path}") from e
    
    def _parse_from_string(self, content: str, format: Optional[str]) -> Dict[str, Any]:
        """Parse schema from string content."""
        try:
            # Simple JSON parsing - would support more formats in production
