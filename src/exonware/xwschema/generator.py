...
Default generation strategy (Option 5: Strategy + composition).
Internal implementation of IGenerationStrategy (ISchemaGenerator).
Used by the engine; not part of public API.
...
class DefaultGenerationStrategy(ISchemaGenerator):
    """
    Default generation strategy (internal). Implements IGenerationStrategy (ISchemaGenerator).
    ...
    async def generate_from_data(self, data: Any, mode: SchemaGenerationMode = SchemaGenerationMode.INFER) -> dict[str, Any]:
        ...
        """
        try:
            # xwdata is the base engine: XWData for structure analysis
    ...
    async def generate_from_xwdata(self, data: XWData, mode: SchemaGenerationMode = SchemaGenerationMode.INFER) -> dict[str, Any]:
        ...
        Fully reuses XWData's efficient navigation for structure analysis.
        XWData provides path-based access and query capabilities for comprehensive analysis.
        """
        # Fully reuse XWData.to_native() for conversion
