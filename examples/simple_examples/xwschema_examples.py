...
Run status: aligned with xwsystem/docs/REPORT_TEST.md (run until all issues solved).
...
# Import XWData (required for examples)
...
async def example_schema_io():
    ...
    # Save schema to file (use temp dir so path security allows it from any cwd)
    schema_path = Path(tempfile.gettempdir()) / 'xwschema_example_schema.json'
    await asyncio.to_thread(schema.to_file, schema_path, format=SchemaFormat.JSON_SCHEMA)
    print(f"Schema saved to {schema_path}")
    # Load schema from file (read + construct to avoid instance-method shadowing in some package versions)
    def _read_schema(p: Path):
        return XWSchema(JsonSerializer().load_file(p))
    loaded_schema = await asyncio.to_thread(_read_schema, schema_path)
...
if __name__ == '__main__':
    try:
        asyncio.run(main())
        print("\n[PASS] All examples completed successfully.")
        sys.exit(0)
    except Exception as e:
        print(f"\n[FAIL] Examples run failed: {e}", file=sys.stderr)
        import traceback
